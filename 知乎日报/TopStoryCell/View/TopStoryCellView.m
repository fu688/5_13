...
@implementation TopStoryCellView
...
-(void)dateRecevid:(NSNotification*)sender {
    ...
    [self likesAndCollectReload:[self.model.top_stories[self.deriction] valueForKey:@"id"]];
    [[Manager shareManager] makeData3:^(story_extralModel * _Nonnull story_extraModel) {
        ...
        //self.likesCountLabel.text = self.likesCountString;
            ...
            NSLog(@"请求失败");
        } andid:[self.model.top_stories[self.deriction] valueForKey:@"id"]];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if ((int)self.topStoriesScrollView.contentOffset.x % (int)widt == 0) {
        ...
        [self likesAndCollectReload:[self.model.top_stories[(int)self.topStoriesScrollView.contentOffset.x / (int)widt] valueForKey:@"id"]];
        ...
        [[Manager shareManager] makeData3:^(story_extralModel * _Nonnull story_extraModel) {
            ...
            self.likesCountLabel.text = self.likesCountString;
                ...
                NSLog(@"请求失败");
            } andid:[self.model.top_stories[(int)self.topStoriesScrollView.contentOffset.x / (int)widt] valueForKey:@"id"]];
